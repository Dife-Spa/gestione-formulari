...
export function DataTable<TData, TValue>({ 
	...
	const [selectedFormulario, setSelectedFormulario] = React.useState<FormularioUI | null>(null)
  ...
  const handleClearDateRange = () => {
    ...
    router.push(`?${createQueryString({ dateFrom: null, dateTo: null, page: 1 })}`)
  }
	...
	const table = useReactTable({
		...
		meta: {
			handleFormularioSelect: (formulario: FormularioUI) => handleFormularioSelect(formulario),
	  ...
	  const handleDocumentsChange = (documents: string[]) => {
		...
		router.push(`?${createQueryString(params)}`)
				...
				const [selectedPecStatus, setSelectedPecStatus] = React.useState<string>(currentPecStatus)
...
const [searchColumn, setSearchColumn] = React.useState<string>(currentSearchColumn)
	...
	<div className="relative">
		...
		<DropdownMenu>
			...
			<DropdownMenuContent align="end" className="w-[200px]">
				...
				<DropdownMenuRadioGroup value={searchColumn} onValueChange={handleSearchColumnChange}>
					<DropdownMenuRadioItem value="">Tutti i campi</DropdownMenuRadioItem>
					...
					<div className="flex items-center gap-2 ml-auto">
						...
						onClick={async () => {
							try {
								...
								const selectedUIDs = selectedRowIndices.map(index => {
								...
								console.log(`Successfully deleted ${selectedUIDs.length} formularios`);
      ...
      <Popover>
        <PopoverTrigger asChild>
          <Button
            id="date"
            variant="outline"
            className={`w-[220px] justify-start text-left font-normal ${!dateRange.from ? "text-muted-foreground" : ""}`} // Changed from w-[300px] to w-[220px]
          >
