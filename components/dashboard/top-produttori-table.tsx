...
export function TopProduttoriTable() {
  const [selectedEntityType, setSelectedEntityType] =
    useState<EntityType>("produttore");
  const [topEntities, setTopEntities] = useState<TopEntity[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const currentOption = entityOptions.find(
    (option) => option.value === selectedEntityType
  )!;
  ...
  async function fetchTopEntities() {
    try {
      ...
      setError(
        `Impossibile caricare i ${currentOption.pluralLabel} più frequenti`
      );
    } finally {
      setIsLoading(false);
    }
          ...
          <DropdownMenuContent align="end">
            {entityOptions.map((option) => (
              <DropdownMenuItem
                ...
                className={
                  selectedEntityType === option.value ? "bg-muted" : ""
                }
              >
                {option.label}
              </DropdownMenuItem>
        ...
        <Table>
          <TableCaption>
            {isLoading
              ...
              ? `Nessun ${currentOption.pluralLabel.slice(0, -1)} trovato`
