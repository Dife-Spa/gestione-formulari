...
import { Calendar, FileText, Truck, Building, User, Link, Check, X, Mail, Pencil, Trash2 } from "lucide-react"
...
export function FormularioDetails({ formulario, open, onOpenChange, onRefresh }: FormularioDetailsProps) {
	// Initialize state variables first, before any conditional returns
	const [editingCodice, setEditingCodice] = React.useState(false);
	const [editingIdAppuntamento, setEditingIdAppuntamento] = React.useState(false);
	const [codiceValue, setCodiceValue] = React.useState("");
	const [idAppuntamentoValue, setIdAppuntamentoValue] = React.useState("");
	const [editingPecDestinatario, setEditingPecDestinatario] = React.useState(false);
	...
	const [riceveFormularioValue, setRiceveFormularioValue] = React.useState(false);
	...
	const [originalIdAppuntamentoValue, setOriginalIdAppuntamentoValue] = React.useState("");
	const [originalPecDestinatarioValue, setOriginalPecDestinatarioValue] = React.useState("");
	const [originalRiceveFormularioValue, setOriginalRiceveFormularioValue] = React.useState(false);
	const [originalRiceveScontrinoValue, setOriginalRiceveScontrinoValue] = React.useState(false);
	...
	const destinazioneInfo = datiFormulario.destinazione_info || datiFormulario.destinatario_info || {};
	...
	const handleSaveCodice = async () => {
		...
		try {
			...
			alert('Errore durante l\'aggiornamento del numero formulario. Verificare che il server API sia in esecuzione.');
	...
	const handleSaveIdAppuntamento = async () => {
		...
		try {
			...
			console.error('Errore durante l\'aggiornamento dell\'ID Appuntamento:', error);
			alert('Errore durante l\'aggiornamento dell\'ID Appuntamento. Verificare che il server API sia in esecuzione.');
					...
					<DrawerHeader className="pb-2 flex flex-row justify-between items-end">
						...
						<div className="flex gap-2">
							<Button 
								...
								title={!datiInvioPec?.pec_destinatario ? "Destinatario PEC non specificato" : "Invia PEC"}
								onClick={async () => {
									if (!formulario?.uid) {
										console.error('UID del formulario non disponibile');
										return;
									...
									try {
										...
										console.error('Dettagli errore:', error instanceof Error ? error.message : String(error));
										alert('Errore durante l\'invio della PEC. Verificare che il server API sia in esecuzione.');
						...
						<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
							...
							<Card className="justify-between">
								<CardHeader>
									...
									<CardDescription>Dettagli del formulario e delle parti coinvolte</CardDescription>
								</CardHeader>
								<CardContent className="space-y-2">
									...
									<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
										...
										<div className="border rounded-md p-3">
											<div className="text-sm text-muted-foreground mb-1">Numero Formulario</div>
											<div className="flex items-center justify-between">
												{editingCodice ? (
													<div className="flex-1 mr-2">
														<Input 
															value={codiceValue} 
															onChange={(e: React.ChangeEvent<HTMLInputElement>) => setCodiceValue(e.target.value)}
												...
												<div className="flex">
													...
													>
														{editingCodice ? <Check className="h-4 w-4" /> : <Pencil className="h-4 w-4" />}
													</Button>
												</div>
											</div>
										</div>
										...
										{/* ID Appuntamento */}
										<div className="border rounded-md p-3">
											<div className="text-sm text-muted-foreground mb-1">ID Appuntamento</div>
											<div className="flex items-center justify-between">
												{editingIdAppuntamento ? (
													<div className="flex-1 mr-2">
														<Input 
															value={idAppuntamentoValue} 
															onChange={(e: React.ChangeEvent<HTMLInputElement>) => setIdAppuntamentoValue(e.target.value)}
															className="font-medium"
														/>
													</div>
												) : (
													<div className="font-medium">{idAppuntamentoValue || "N/A"}</div>
												)}
												<div className="flex">
													{editingIdAppuntamento && (
														<Button 
													...
													>
														{editingIdAppuntamento ? <Check className="h-4 w-4" /> : <Pencil className="h-4 w-4" />}
													</Button>
												</div>
											</div>
										</div>
									...
									<div className="flex justify-between items-center py-3 mb-0 border-b">
										<span className="text-xs font-medium text-muted-foreground">Data Emissione</span>
									...
									<div className="py-3 mb-0 border-b">
										<div className="flex justify-between items-center">
											<span className="text-xs font-medium text-muted-foreground">Produttore</span>
											<span className="text-xs">{formulario.produttore}</span>
										</div>
										<div className="text-xs text-muted-foreground text-right mt-1">
											{formulario.unita_locale_produttore || 'Indirizzo non disponibile'}
										</div>
									</div>
									...
									<div className="py-3 mb-0 border-b">
										...
										<div className="text-xs text-muted-foreground text-right mt-1">
											{formulario.unita_locale_destinatario || 'Indirizzo non disponibile'}
										</div>
									</div>
									
									{/* Trasportatore */}
									<div className="flex justify-between items-center py-3 mb-0 border-b">
										<span className="text-xs font-medium text-muted-foreground">Trasportatore</span>
										<span className="text-xs">{formulario.trasportatore || "Non specificato"}</span>
									</div>
									
									{/* Intermediario */}
									<div className="flex justify-between items-center py-3 mb-0 border-b">
										<span className="text-xs font-medium text-muted-foreground">Intermediario</span>
										<span className="text-xs">{formulario.intermediario || "Non specificato"}</span>
									</div>
									
									{/* ID Univoco */}
									<div className="flex justify-between items-center py-3 mb-0 border-b">
										<span className="text-xs font-medium text-muted-foreground">ID Univoco</span>
										<span className="text-xs">{formulario.uid}</span>
									</div>
									
									{/* Separator for Caratteristiche Rifiuto section */}
									<div className="pt-2">
										<h3 className="font-medium text-sm my-2">Caratteristiche Rifiuto</h3>
										
										{/* Codice EER */}
										<div className="flex justify-between items-center py-3 mb-0 border-b">
											<span className="text-xs font-medium text-muted-foreground">Codice EER</span>
											<span className="text-xs">{caratteristicheRifiuto.eer || "Non specificato"}</span>
										</div>
										
										{/* Quantità */}
										<div className="flex justify-between items-center py-3 mb-0 border-b">
											<span className="text-xs font-medium text-muted-foreground">Quantità</span>
											<span className="text-xs">{datiAppuntamento?.quantitaRealeKg || caratteristicheRifiuto.quantita || formulario.quantita || "-"} {caratteristicheRifiuto.unita_misura || "kg"}</span>
										</div>
										
										{/* Stato Fisico */}
										<div className="flex justify-between items-center py-3 mb-0 border-b">
											<span className="text-xs font-medium text-muted-foreground">Stato Fisico</span>
											<span className="text-xs">{caratteristicheRifiuto.stato_fisico || "Non specificato"}</span>
										</div>
										
										{/* Aspetto */}
										<div className="flex justify-between items-center py-3 mb-0 border-b">
											<span className="text-xs font-medium text-muted-foreground">Aspetto</span>
											<span className="text-xs">{caratteristicheRifiuto.aspetto || "Non specificato"}</span>
										</div>
										
										{/* Peso Verificato Partenza */}
										<div className="flex justify-between items-center py-3 mb-0">
											<span className="text-xs font-medium text-muted-foreground">Peso Verificato Partenza</span>
											<span className="text-xs">{caratteristicheRifiuto.peso_verificato_partenza || "Non specificato"}</span>
										</div>
									</div>
								</CardContent>
							</Card>
							...
							<Card className="justify-between">
								<CardHeader>
									...
									<CardDescription>Informazioni sul trasporto e destinazione</CardDescription>
								</CardHeader>
								<CardContent className="space-y-4">
									...
									<div>
										...
										{/* Conducente */}
										<div className="flex justify-between items-center py-3 mb-0 border-b">
											<span className="text-xs font-medium text-muted-foreground">Conducente</span>
											<span className="text-xs">{trasporto.conducente || "Non specificato"}</span>
										</div>
										
										{/* Data Inizio */}
										<div className="flex justify-between items-center py-3 mb-0 border-b">
											<span className="text-xs font-medium text-muted-foreground">Data Inizio</span>
											<span className="text-xs">{trasporto.data_inizio || formattedDate}</span>
										</div>
										
										{/* Ora Inizio */}
										<div className="flex justify-between items-center py-3 mb-0 border-b">
											<span className="text-xs font-medium text-muted-foreground">Ora Inizio</span>
											<span className="text-xs">{trasporto.ora_inizio || "Non specificato"}</span>
										</div>
										
										{/* Targa Automezzo */}
										<div className="flex justify-between items-center py-3 mb-0 border-b">
											<span className="text-xs font-medium text-muted-foreground">Targa Automezzo</span>
											<span className="text-xs">{trasporto.targa_automezzo || "Non specificato"}</span>
										</div>
										
										{/* Targa Rimorchio */}
										<div className="flex justify-between items-center py-3 mb-0 border-b">
											<span className="text-xs font-medium text-muted-foreground">Targa Rimorchio</span>
											<span className="text-xs">{trasporto.targa_rimorchio || "Non presente"}</span>
										</div>
									</div>
									
									{/* Dati Destinazione Section */}
									<div>
										<h3 className="text-sm font-medium mb-3">Dati Destinazione</h3>
										
										{/* Data Arrivo */}
										<div className="flex justify-between items-center py-3 mb-0 border-b">
											<span className="text-xs font-medium text-muted-foreground">Data Arrivo</span>
											<span className="text-xs">{destinazioneInfo.data_arrivo || formattedDate}</span>
										</div>
										
										{/* Ora Arrivo */}
										<div className="flex justify-between items-center py-3 mb-0 border-b">
											<span className="text-xs font-medium text-muted-foreground">Ora Arrivo</span>
											<span className="text-xs">{destinazioneInfo.ora_arrivo || "Non specificato"}</span>
										</div>
										
										{/* Carico Accettato */}
										<div className="flex justify-between items-center py-3 mb-0 border-b">
											<span className="text-xs font-medium text-muted-foreground">Carico Accettato</span>
											<span className="text-xs">{destinazioneInfo.carico_accettato || "Non specificato"}</span>
										</div>
										
										{/* Quantità Accettata */}
										<div className="flex justify-between items-center py-3 mb-0 border-b">
											<span className="text-xs font-medium text-muted-foreground">Quantità Accettata</span>
											<span className="text-xs">{destinazioneInfo.quantita_accettata || "Non specificato"}</span>
										</div>
										
										{/* Quantità Respinta */}
										<div className="flex justify-between items-center py-3 mb-0 border-b">
											<span className="text-xs font-medium text-muted-foreground">Quantità Respinta</span>
											<span className="text-xs">{destinazioneInfo.quantita_respinta || "Non specificato"}</span>
										</div>
										
										{/* Attesa Verifica */}
										<div className="flex justify-between items-center py-3 mb-0 border-b">
											<span className="text-xs font-medium text-muted-foreground">Attesa Verifica</span>
											<span className="text-xs">{destinazioneInfo.attesa_verifica || "Non specificato"}</span>
										</div>
										
										{/* Motivazioni */}
										<div className="flex justify-between items-center py-3 mb-0">
											<span className="text-xs font-medium text-muted-foreground">Motivazioni</span>
											<span className="text-xs">{destinazioneInfo.motivazioni || "Non specificato"}</span>
										</div>
									</div>
								</CardContent>
							</Card>
							...
							<div className="flex flex-col gap-4">
								...
								<Card>
									<CardHeader>
										...
										<CardDescription>Informazioni per l'invio PEC</CardDescription>
									</CardHeader>
									<CardContent className="space-y-4">
										{/* Destinatario PEC */}
										<div>
											<h4 className="text-sm font-medium mb-2">Destinatario PEC</h4>
												...
												{editingPecDestinatario ? (
													<div className="flex-1 mr-2">
														<Input 
															...
															onChange={(e: React.ChangeEvent<HTMLInputElement>) => setPecDestinatarioValue(e.target.value)}
															className="text-sm"
														/>
													</div>
												) : (
													<span className={`text-sm ${pecDestinatarioValue ? '' : 'text-muted-foreground'}`}>
														{pecDestinatarioValue || "Indirizzo PEC non specificato"}
													</span>
												)}
										...
										<div className="border rounded-md p-4">
											<div className="flex items-center justify-between mb-3">
												<h4 className="text-sm font-medium">Opzioni di Invio</h4>
												<Button 
													variant="ghost" 
													size="icon" 
													className="h-6 w-6"
												...
												>
													{editingPecOptions ? <Check className="h-4 w-4" /> : <Pencil className="h-4 w-4" />}
												</Button>
											</div>
											...
											<div className="flex items-center justify-between mb-3 border rounded-md p-3">
												<div>
													<p className="text-sm font-medium">Riceve Formulario</p>
													<p className="text-xs text-muted-foreground">Il destinatario riceverà il file del formulario via PEC</p>
												</div>
												<Switch
													checked={editingPecOptions ? riceveFormularioValue : (datiInvioPec.riceveFormulario || false)}
													onCheckedChange={editingPecOptions ? setRiceveFormularioValue : undefined}
											...
											<div className="flex items-center justify-between border rounded-md p-3">
												<div>
													<p className="text-sm font-medium">Riceve Scontrino</p>
													<p className="text-xs text-muted-foreground">Il destinatario riceverà il file dello scontrino via PEC</p>
												</div>
												<Switch
													checked={editingPecOptions ? riceveScontrinoValue : (datiInvioPec.riceveScontrino || false)}
													onCheckedChange={editingPecOptions ? setRiceveScontrinoValue : undefined}
													disabled={!editingPecOptions}
													id="riceve-scontrino"
								...
								<Card className="h-full justify-between">
									<CardHeader>
										...
										<CardDescription>Documenti disponibili per questo formulario</CardDescription>
									</CardHeader>
									<CardContent>
										<div className="flex flex-col gap-3">
											...
											<a 
												...
												onClick={(e) => !formulario.file_paths?.file_input && e.preventDefault()}
											>
											...
											<a 
												...
												onClick={(e) => !formulario.file_paths?.formulario && e.preventDefault()}
											>
												<FileText className="h-4 w-4" />
												Formulario
											</a>
											...
											<a 
												...
												onClick={(e) => !formulario.file_paths?.buono_intervento && e.preventDefault()}
											>
												<FileText className="h-4 w-4" />
												Buono Intervento
											</a>
											...
											<a 
												...
												onClick={(e) => !formulario.file_paths?.scontrino && e.preventDefault()}
											>
												<FileText className="h-4 w-4" />
												Scontrino
											</a>
			...
			<Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							Sei sicuro di voler eliminare questo formulario? Questa azione non è reversibile.
						</DialogDescription>
					</DialogHeader>
					<DialogFooter>
